...
local GSA_VERSION = "|cffFF7D0A Wrath-1.5 |r(|cff4DFF4D3.4.0 Wrath of the Lich King (Classic)|r)"
...
local function RegisterSound(soundName, soundPath)
    if (type(soundName) ~= "string" or type(soundPath) ~= "string") then
        return
    end
    if (soundName == "" or soundPath == "") then
        return
    end
...
for soundName, soundPath in pairs(LSM_GSA_SOUNDFILES) do
    RegisterSound(soundName, soundPath)
end
for index, soundName in pairs(LSM:List("sound")) do
    soundz[soundName] = LSM:Fetch("sound", soundName)
end
...
function GladiatorlosSA:OnEnable()
    ...
    if not GSA_LANGUAGE[gsadb.path] then
        gsadb.path = GSA_LOCALEPATH[GetLocale()]
    end
    self.throttled = {}
    self.smarter = 0
end
...
function GladiatorlosSA:PlaySpell(listName, spellID, sourceGUID, destGUID, ...)
    ...
    if not gsadb[list[spellID]] then
        return
    end
    ...
    if gsadb.smartDisable then
        if (GetNumGroupMembers() or 0) > 20 then
            return
        end
...
function GladiatorlosSA:COMBAT_LOG_EVENT_UNFILTERED(event, ...)
    ...
    if (isSanctuary == "sanctuary") then
        return
    end    -- Checks for Sanctuary
    if (not canSpeakHere) then
        return
    end                -- Checks result for everywhere else
    ...
    elseif (event == "SPELL_CAST_SUCCESS" and sourcetype[COMBATLOG_FILTER_HOSTILE_PLAYERS] and (not gsadb.sonlyTF or sourceuid.target or sourceuid.focus) and not gsadb.castSuccess) then
        if self:Throttle(tostring(spellID) .. "default", 0.05) then
            return
        end
...
function GladiatorlosSA:UNIT_AURA(event, uid)
    ...
    if UnitIsEnemy("player", uid) and gsadb.drinking then
        if (AuraUtil.FindAuraByName("Drinking", uid) or AuraUtil.FindAuraByName("Food", uid) or AuraUtil.FindAuraByName("Refreshment", uid) or AuraUtil.FindAuraByName("Drink", uid)) and currentZoneType == "arena" then
            if self:Throttle(tostring(104270) .. uid, 4) then
                return
            end
