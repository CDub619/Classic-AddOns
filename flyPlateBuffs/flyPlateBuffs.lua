...
local function Nameplate_Added(...)
	...
	if unitType == "Creature" or unitType == "Vehicle" or unitType == "Pet" and UnitIsEnemy("player" , nameplateID) then --or unitType == "Pet"  then
...
local castedAuraIds = {
	--[33831] = 30, --Trees **Need to Change to Show 1 Icon for all 3 (Later)
	--[34433] = 15, --Shadowfiend
	--[58833] = 30, --Mirror Image **Need to Change to Show 1 Icon for all 3 (Later)
	--[58831] = 30, --Mirror Image **Need to Change to Show 1 Icon for all 3 (Later)
	--[58834] = 30, --Mirror Image **Need to Change to Show 1 Icon for all 3 (Later)
	--[31687] = 45, --Water Ele {Tested)
	--[51533] = 45, --Feral Spirits **Need to Change to Show 1 Icon for all 2 (Later)
	--Casted
	[23989] = 3, --Readiness
	[14185] = 3, --Preparation
...
-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
		...
		if ((event == "SPELL_SUMMON") or (event == "SPELL_CREATE"))  then --Summoned CDs
			if castedAuraIds[spellId] and sourceGUID and (bit_band(sourceFlags, COMBATLOG_OBJECT_REACTION_HOSTILE) == COMBATLOG_OBJECT_REACTION_HOSTILE) then
				...
				if spellId == 55342 then --Mirror Image Icon Change
					icon = 135994
				end
		...
		-----------------------------------------------------------------------------------------------------------------
			if (event == "SPELL_CAST_SUCCESS") and ((spellId == 23989) or (spellId == 14185)) then --Readiness & Prep & Wele
			if castedAuraIds[spellId] and sourceGUID and (bit_band(sourceFlags, COMBATLOG_OBJECT_REACTION_HOSTILE) == COMBATLOG_OBJECT_REACTION_HOSTILE) then
				local duration = castedAuraIds[spellId]
				local type = "HARMFUL"
				local namePrint, _, icon = GetSpellInfo(spellId)
		...
		--[[Warrior Stances
		-----------------------------------------------------------------------------------------------------------------
		if (event == "SPELL_CAST_SUCCESS") and ((spellId == 71) or (spellId == 2457) or (spellId == 2458)) then
			if castedAuraIds[spellId] and sourceGUID and not (bit_band(sourceFlags, COMBATLOG_OBJECT_REACTION_HOSTILE) == COMBATLOG_OBJECT_REACTION_HOSTILE) then
				...
				if Interrupted[sourceGUID] then
					for k, v in pairs(Interrupted[sourceGUID]) do
						if v.spellId ~= spellId and (v.spellId == 71 or v.spellId == 2457 or v.spellId ==2458) then
							print(sourceName.." Stance Changed to "..namePrint.." from "..v.namePrint.." fPB")
							Interrupted[sourceGUID][k] = nil
						end
					end
				end
